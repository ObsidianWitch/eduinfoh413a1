...
int main (int argc, char **argv) {
     ...
     * so it is constant per-instance, but (most likely) varies between
     * instances. */
    Seed = (long int) 0;
    for (i = 0 ; i < PSize ; ++i) {
        for (j = 0 ; j < PSize ; ++j) {
    ...
    /* Create an initial random solution. The only constraint is that it should
     * always be a permutation */
     ...
     * TODO There are some more efficient way to do this, instead of recomputing
     * everything... */
